...
var getUniversitiesFromDb = RepoFunction.getUniversitiesFromDb; // mongoose.connect(process.env.CLUSTER_URL, {
...
var makepdfs = function makepdfs(attendance, year, month, id) {
  ...
  pdfDoc.pipe(fs.createWriteStream("attendance/" + monthNames[month - 1] + "_" + year + "_" + id + ".pdf"));
  pdfDoc.end();
};
...
var sendpdf = function sendpdf() {
  ...
  return getUniversitiesFromDb().then(function (universities) {
    universities.forEach(function (university) {
      getAttendance(university._id, year, month).then(function (attendance) {
        makepdfs(attendance, year, month, university._id); // db.close();
      });
    });
  });
};
